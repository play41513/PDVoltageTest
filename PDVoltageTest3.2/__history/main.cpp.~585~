	...
	: TForm(Owner)
{
	...
	mLogMsg->AddLocalFilePath("PDVotageTool");
}
void TFrmMain::UI_Update()
{
	if(!mainThread->queUIUpdate.empty())
	{
		UI_UPDATE_DESCRIPOR updateDescriptor;
		updateDescriptor = mainThread->queUIUpdate.front();
		switch(updateDescriptor.UI_Msg)
		{
			case WM_UI_PANEL_RESULT:
				switch(updateDescriptor.UI_WParam)
				{
			...
			case WM_UI_PANEL_NOLOAD:
				...
				INI_Desc.PITEM_PD_DESCRIPOR[updateDescriptor.UI_LParam].plNoLoadValue->Caption
					 = INI_Desc.PITEM_PD_DESCRIPOR[updateDescriptor.UI_LParam].NoLoadVoltageNow;
				switch(updateDescriptor.UI_WParam)
				{
					case UI_PANEL_FONT_GREEN:
						INI_Desc.PITEM_PD_DESCRIPOR[updateDescriptor.UI_LParam].plNoLoadValue
							->Font->Color = clGreen;
						break;
			...
			case WM_UI_PANEL_FULLLOAD:
				INI_Desc = mainThread->ReadINIDesc();
				INI_Desc.PITEM_PD_DESCRIPOR[updateDescriptor.UI_LParam].plFullLoadValue->Caption
					 = INI_Desc.PITEM_PD_DESCRIPOR[updateDescriptor.UI_LParam].FullLoadVoltageNow;
				switch(updateDescriptor.UI_WParam)
				{
					case UI_PANEL_FONT_GREEN:
						INI_Desc.PITEM_PD_DESCRIPOR[updateDescriptor.UI_LParam].plFullLoadValue
							->Font->Color = clGreen;
						break;
					case UI_PANEL_FONT_RED:
						INI_Desc.PITEM_PD_DESCRIPOR[updateDescriptor.UI_LParam].plFullLoadValue
				...
				break;
			case WM_UI_PANEL_E_MARK:
				switch(updateDescriptor.UI_WParam)
				{
					case UI_PANEL_FONT_GREEN:
						plE_MarkResult->Caption = "O";
						plE_MarkResult->Font->Color = clGreen;
						break;
					case UI_PANEL_FONT_RED:
						plE_MarkResult->Caption = "X";
						plE_MarkResult->Font->Color = clRed;
						break;
					case UI_PANEL_FONT_BLUE:
						plE_MarkResult->Caption = "-";
						plE_MarkResult->Font->Color = clBlack;
						break;
				}
				break;
		}
		mainThread->queUIUpdate.pop();
	}
}
...
bool TFrmMain::ReadINIParm()
{
	...
	{
		plE_Mark->Visible = true;
		plE_MarkTitle->Caption = INI_Desc.E_MarkUIDesc;
		plE_MarkTitle->Font->Size = INI_Desc.E_MarkFontSize;
		plE_MarkResult->Caption = "-";
	}
	return true;
}
