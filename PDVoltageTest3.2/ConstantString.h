...
#define DEBUG_TEST_START_VOLTAGE_AUTO   		"[TEST_START_VOLTAGE_AUTO]"
...
#define WM_UI_PANEL_E_MARK              WM_USER+103
...
typedef struct _ITEM_PD_DESCRIPOR
{
  ...
  bool bWattIsOver;
  //
  AnsiString NoLoadVoltageNow;
  AnsiString FullLoadVoltageNow;
  //
  DWORD dwLoadTime;
...
typedef struct _E_MARK_DESCRIPOR
{
  bool bEnable;
  AnsiString strINI_Voltage;
  AnsiString strINI_Current;

  AnsiString strEMark_Voltage;
  AnsiString strEMark_Current;
  bool bPass;
...
typedef struct _INI_DESCRIPOR
{
  ...
  E_MARK_DESCRIPOR PE_MARK_DESCRIPOR[5];
  ...
  AnsiString E_MarkUIDesc;
  DWORD      E_MarkFontSize;
  bool 		 bE_MarkPass;
  bool 		 bE_MarkEnable;

}INI_DESCRIPOR;

typedef struct _UI_UPDATE_DESCRIPOR
{
  DWORD UI_Msg;
  DWORD UI_WParam;
  DWORD UI_LParam;
