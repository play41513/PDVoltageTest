...
INI_DESCRIPOR cFileControl::ReadINIFile()
{
	...
	INI_Desc.dwDelayTime_Auto_VPID = ini->ReadInteger("DUT_VPID_AUTO_TEST","DelayTime",0);
	//
	...
	//
	INI_Desc.bE_MarkEnable = false;
	INI_Desc.E_MarkUIDesc
		= ini->ReadString("EMARK","UIDesc","E-MARK");
	INI_Desc.E_MarkFontSize
		= ini->ReadInteger("EMARK","UIFontSize",20);
	//
	for(int i = 0 ; i < 5 ; i++)
	{
		...
		//EMARK
		INI_Desc.PE_MARK_DESCRIPOR[i].bEnable
			= ini->ReadBool("EMARK_"+AnsiString(i+1),"Enable",false);
		if(INI_Desc.PE_MARK_DESCRIPOR[i].bEnable)
		{
			INI_Desc.bE_MarkEnable = true;
			INI_Desc.PE_MARK_DESCRIPOR[i].strINI_Voltage
				= ini->ReadString("EMARK_"+AnsiString(i+1),"RequestedVoltage","20");
			INI_Desc.PE_MARK_DESCRIPOR[i].strINI_Current
				= ini->ReadString("EMARK_"+AnsiString(i+1),"Current","5");
			INI_Desc.PE_MARK_DESCRIPOR[i].strEMark_Current = "";
			INI_Desc.PE_MARK_DESCRIPOR[i].bPass = false;
        }
	}
	return INI_Desc;
}
