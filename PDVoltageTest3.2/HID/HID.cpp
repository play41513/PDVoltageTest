...
void HID::SetAutoVoltageValue(double value)
{
	dbAutoVoltage = value;
}
bool HID::HID_READ_VALUE()
{
		...
		switch(step)
		{
			...
			case HID_TURN_ON:
				if(HID_TurnOn())
					...
					step = HID_ERROR;
				break;
			case HID_READ_VOLTAGE:
				Tx(ReadADValue);
				if(Rx(""))
					step = HID_CHECK_VOLTAGE;
				else
					step = HID_ERROR;
				break;
			case HID_CHECK_VOLTAGE:
				if(AD_VALUE.ToDouble()>=dbAutoVoltage)
					return true;
				return false;
				break;
			case HID_ERROR:
...
bool HID::HID_Control(AnsiString CMD)
{
	int step = HID_IS_ONLINE;
	bool bl = true;
	while(bl)
...
bool HID::HID_READ_EMARK()
{
	int step = HID_IS_ONLINE;
	bool bl = true;
	HID_READ_E_MARK_VALUE = "";
	while(bl)
	{
		switch(step)
		{
...
bool HID::HID_TurnOn()
{
	...
	if(Configure(HID_PID,HID_VID,L"",L"")){
		if(!Open()){
			//MessageBoxA(NULL, "開啟 PD 板失敗!", "", MB_ICONWARNING);
			return false;
        }
	}else{
		//MessageBoxA(NULL, "配置 PD 板失敗!", "", MB_ICONWARNING);
		return false;
	}
	HID_PID[4] = 0;
	HID_PID[4] = 0;
...
bool HID::HID_ReadReport(AnsiString Value)
{
	...
	if(Value!="")
	{
		if(Value == "EMARK")
		{
			if(buf3.Pos(ReadEMarkValue))
				HID_READ_E_MARK_VALUE = buf3;
        }
		else if(!strcmp(buf3.SubString(8,Value.Length()).c_str(),Value.c_str()))
			return true;
		else return false;
	}
	else
...
void HID::Rx_ValueAnalyze(AnsiString Value){
	...
	AD_VALUE.printf("%.3f",float(HexToInt(HexValue))*UNIT);
}
