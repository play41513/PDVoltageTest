	...
	while(bl)
	{
		switch(step)
		{
			...
			case READ_AD_VALUE:
				Tx(ReadADValue);
				Rx("");
				if(atof(m_ADValue.c_str())>= MIN_VOLTAGE_TOLERANCE_RANGE[numBtn-1] && atof(m_ADValue.c_str()) <= MAX_VOLTAGE_TOLERANCE_RANGE[numBtn-1])
		...
		if(bl)
		{
			...
			for(int i=1 ;i<=5 ;i++)
			{
				if(reg->ValueExists("VoltageToleranceRange_"+IntToStr(i))){
					...
					((TPanel *)FindComponent("pl_ckb"+IntToStr(i)+"_1"))->Caption = value.SubString(1,1).ToInt()?"V":"";
					((TEdit *)FindComponent("edt_Pi"+IntToStr(i)))->Text    = value.SubString(3,value.Pos("&")-3);
					((TPanel *)FindComponent("pl_ckb"+IntToStr(i)+"_2"))->Caption = value.SubString(value.Pos("&")+1,1).ToInt()?"V":"";
					((TEdit *)FindComponent("edt_min"+IntToStr(i)))->Text	  = value.SubString(value.Pos("&")+3,value.Pos("、")-(value.Pos("&")+3));
...
void TFrmMain::SetRegVal(AnsiString numTestItem,bool AutoTest,AnsiString DutVPID){
	...
	try {
		...
		for(int i =1 ;i <=5 ;i++)
			VoltageRange[i-1] =
				(((TPanel *)FindComponent("pl_ckb"+IntToStr(i)+"_1"))->Caption == "V"?"1_":"0_")
				+ AnsiString(((TEdit *)FindComponent("edt_Pi"+IntToStr(i)))->Text)
				+ "&"
				+ (((TPanel *)FindComponent("pl_ckb"+IntToStr(i)+"_2"))->Caption == "V"?"1_":"0_")
...
void  TFrmMain::SetValueEnabled(bool Enabled)
{
	...
	for(int i=1 ; i<=5 ;i++)
	{
		...
		if(Enabled)
		{
			TPanel  * plckb   = ((TPanel *)FindComponent("pl_ckb"+IntToStr(i)+"_1"));
			if(plckb->Caption == "V")
			{
				((TEdit *)FindComponent("edt_Pi"+IntToStr(i)))->Enabled = true;
				((TPanel *)FindComponent("plPI"+IntToStr(i)))->Font->Color = clBlue;
				((TEdit *)FindComponent("edt_min"+IntToStr(i)))->Enabled = false;
				((TEdit *)FindComponent("edt_max"+IntToStr(i)))->Enabled = false;
			}
			else
			{
				((TEdit *)FindComponent("edt_Pi"+IntToStr(i)))->Enabled = false;
				((TPanel *)FindComponent("plPI"+IntToStr(i)))->Font->Color = clGray;
				((TEdit *)FindComponent("edt_min"+IntToStr(i)))->Enabled = true;
				((TEdit *)FindComponent("edt_max"+IntToStr(i)))->Enabled = true;
			}
		}
		else
		{
...
void __fastcall TFrmMain::pl_ckb1_1Click(TObject *Sender)
{
	TPanel* pl=(TPanel*)Sender;
	pl->Caption = pl->Caption == "V" ? "":"V";
	AnsiString plName =pl->Name.SubString(1,pl->Name.Length()-1);
	if(pl->Tag == 1)  ((TPanel *)FindComponent(plName + 2))->Caption = pl->Caption == "V" ? "":"V";
	else ((TPanel *)FindComponent(plName + 1))->Caption = pl->Caption == "V" ? "":"V";
...
void __fastcall TFrmMain::edt_Pi1Change(TObject *Sender)
{
	...
	if(bPASS)
	{
		if(edt->Text.Length()>2)
			edt->Text = edt->Text.SubString(1,2);
		FormResize(NULL);
	}
