...
int TimeStart = 0,DelayTime20V = 0;
bool bTesting = false;
AnsiString DUT_MAC= "";
	...
	: TForm(Owner)
{
	...
	LOG_FILE_PATH += AnsiString(FormatDateTime("\\yyyy-mm-dd_hh-nn-ss", Now()))+".log";
	FormResize(NULL);
			...
			for(int i = 0;i < 5;i++)
			{
				...
				else if(i==4)
				{
					Temp = " ["+AnsiString(zVoltageSetting[i]->Tag)+"V]"
						+zRead_AD_Voltage[i]->Caption+"(V)"+zRead_FULL_LOAD_Voltage[i]->Caption+"(V)";
					SS += Temp;
					Delay(100);
					bTesting = true;
