...
int cmd_get_system_consumptoin(char *s)
{
	...
	{   /*
		...
		*/
		...
		{
			if(FrmMain->bTEST_END_SWITCH_5V)
			{
				if(dwTimeOut == 0)
		...
		while(step)
		{
			switch(step)
			{
				...
				case RS232_READ:
					...
					else if(strstr(FrmMain->que_cmd.front().c_str(),"rise?"))
					{
						DWORD ByteRead;
						char RXBuff[20];
						char buff[20];
					...
					else
					{
						...
						{
							FrmMain->Delay(200); // LOAD ON Delay
							bLoad = true;
							step = RS232_WRITE;
						}
						else if(strstr(FrmMain->que_cmd.front().c_str(),"LOAD OFF"))
						{
							FrmMain->Delay(100); // LOAD OFF Delay
							bLoad = false;
							step = TEST_END;
						}
						else
...
AnsiString  cCOM::ReadRise()
{
	unsigned long Written ;
	int step = RS232_WRITE ;
	dwTimeOut = 0;
	AnsiString SS_cmd,SS="NULL";
	try
	{
		while(step)
		{
			switch(step)
			{
				...
				//寫
					bNoResponse = false;
					SS_cmd =  "*remote;chan 1;rise?\n";
					DEBUG(SS_cmd);
					WriteFile(DEV_hCom,SS_cmd.c_str(),SS_cmd.Length(),&Written,NULL);
					cmd_TimeOut=GetTickCount()+1000;
					//DEBUG("time(ms): "+(AnsiString)(GetTickCount()-frmMain->init_time));
					step=RS232_READ;
					break;
...
//開COM
	...
	//讀
		...
		if (ByteRead>0) // 接收到數據
		{
			strncpy(buff,RXBuff,ByteRead);
			DEBUG(buff);
			if(strstr(buff,"3315F")||strstr(buff,"3310F")||strstr(buff,"3311F")||strstr(buff,"3312F")||strstr(buff,"3313F")||strstr(buff,"3314F")||strstr(buff,"3316G"))
			{
