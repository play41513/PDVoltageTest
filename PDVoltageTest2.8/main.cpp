...
int TimeStart = 0,DelayTimeBack5V = 0,DelayTime20V = 0,DelayTimeBack5V_pre = 0;
bool bTesting = false;
AnsiString DUT_MAC= "";
...
bool TFrmMain::ReadInISet()
{
	...
	//
	value = Findfilemsg(FILE_DUT_SET_INI,"[DelayTimeBack5V_Pre]",1);
	if(value != "")
		DelayTimeBack5V_pre = value.ToInt();
	//
...
void TFrmMain::SetInIVal(AnsiString numTestItem,bool AutoTest,AnsiString DutVPID){
	...
	//
	fp << "[DelayTimeBack5V_Pre]" << endl;
	fp << AnsiString(DelayTimeBack5V_pre).c_str() << endl;
	//
